...
function parseTauriError(error: unknown): ConstructorParameters<typeof AppError> {
  ...
  if (error && typeof error === "object") {
    ...
    if (err.code && typeof err.code === "string") {
      return [err.code as ErrorCode, err.details];
    }
  }
  ...
  if (error instanceof Error) {
    try {
      ...
      if (parsed.code) {
        return [parsed.code as ErrorCode, parsed.details];
      }
    } catch {
      return ["unknown" as ErrorCode, error.message];
    }
  }
  ...
  if (typeof error === "string") {
    try {
      ...
      return ["unknown" as ErrorCode, error];
    }
  }
 ...
 * @returns result of tauri invoke command
 * @throws any error from tauri side
 */
