...
- **Desktop source of truth**: `crates/koloda/src/migrations/`.
...
5. **Hand-write the matching Refinery migration** at `crates/koloda/src/migrations/V<next>__<name>.sql`.
...
7. **Verify**: `cargo check -p koloda` so the embedded migrations compile.
