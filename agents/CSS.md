...
- `animate-shimmer` — mask sweep for mixed icon+text rows. Use `animate-shimmer-text--fg-level-4/fg-level-1` for text-only (pending labels). Use `animate-shimmer-icon` on a second copy of an icon stacked on a solid base when the label is `TextSwap`; do not put `animate-shimmer` on that label's ancestor.
