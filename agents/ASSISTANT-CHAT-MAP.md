...
| Change run lifecycle (start/cancel/fail) | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Runs) | `assistant/state/conversation-reducer.ts` (`transitionRun`, `submitTurn`), `runs/use-conversation-runs.ts`, `runs/use-run-orchestration.ts` | A run has exactly one user msg and one assistant msg. Submit creates both + the run in one `submitTurn` dispatch. Status transitions go through `transitionRun` (streaming → success\|failed\|canceled\|interrupted; restart → streaming); illegal terminals are no-ops. Stream transport + run execution live in `use-conversation-runs.ts` (per-run AbortController; not ai-react stream hooks). |
...
| Change persistence / restore | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Persistence) | `assistant/persistence/` (`use-conversation-save-host`, `use-conversation-restore`, composer, `create-conversation-save-queue`, `create-save-scheduler`, `conversation-persistence`); drizzle/, crates/koloda-core/src/repo | Failed runs must not break conversation persistence. Autosave is route-scoped (`useConversationSaveHost`) with per-conversation serialized queues; restore no longer depends on saver-before-restore effect order. |
