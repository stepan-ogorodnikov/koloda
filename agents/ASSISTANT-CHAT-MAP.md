...
| Change persistence / restore | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Persistence) | `assistant/persistence/` (`use-conversation-restore`, `use-conversation-saver`, composer, `create-save-scheduler`, `conversation-persistence`); drizzle/, crates/koloda-core/src/repo | Failed runs must not break conversation persistence. Composer must call saver before restore (effect order). |
| Choose conversation dispatch flavor | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Persistence, §During Streaming, §Revert) | `assistant/runs/use-assistant-session.ts` (helpers), `runs/use-conversation-runs.ts`, `runs/use-run-orchestration.ts` | Keep three named helpers — do not collapse into one options bag. `dispatch`: current convo + `touch()` (start/cancel/commit). `dispatchToConversation(id)`: by-id, no auto-touch (stream chunks; terminal success/abort calls `touch()` explicitly). `dispatchLocal`: current convo, no save (in-memory revertState). |
