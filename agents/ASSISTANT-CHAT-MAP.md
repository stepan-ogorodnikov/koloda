...
| Choose conversation dispatch flavor | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Persistence, §During Streaming, §Revert) | libs/srs-react/src/lib/assistant/use-assistant-session.ts (helpers), use-conversation-runs.ts, use-run-orchestration.ts | Keep three named helpers — do not collapse into one options bag. `dispatchPersisted`: current convo + bump save (start/cancel/commit). `dispatchToConversation(id)`: by-id, no auto-bump (stream chunks; terminal success/abort bumps explicitly). `dispatchEphemeral`: current convo, no save (in-memory revertState). |
