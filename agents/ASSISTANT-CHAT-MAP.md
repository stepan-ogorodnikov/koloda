...
| `runs/` | Session (`RunController`), orchestration, stream execution, `prepare-run-request` / `build-stream-request` |
...
| Change persistence / restore | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Persistence, §Restore) | `@koloda/assistant` (`create-conversation-save-queue`, `create-save-scheduler`, `conversation-persistence-host`, `assistant-observability`, `SHUTDOWN_FLUSH_TIMEOUT_MS`, `SHUTDOWN_SAVE_MAX_ATTEMPTS`); `assistant/persistence/` (`use-conversation-save-host`, `use-conversation-restore`, `conversation-persistence`, `conversation-persistence-schema`, `conversation-schema-version`); `assistant/runs/use-assistant-engine-host.ts` (engine + persistence singleton, graceful shutdown, `deleteAssistantConversation`); `@koloda/app-react` `components/app.tsx` (shell-scoped host mount); Electron close handshake `apps/native-electron/src/window-close-coordinator.ts` + `apps/native-electron-react/.../electron-close-coordination.ts`; drizzle/, crates/koloda-core/src/repo | Failed runs must not break conversation persistence. Persisted rows carry `schemaVersion`; unknown future versions fail coerce. Strict run lifecycle validation rejects illegal status/reason pairs. On restore, streaming checkpoints become `interrupted`/`crash_recovery` with partial output kept (not removed). Per-conversation save queues and shutdown flush are engine-owned (`useAssistantEngineHost` + `useConversationSaveHost` write adapter) at **application-shell** scope; AI-route/chat unmount does not dispose queues, abort runs, or drop unload listeners. Failed saves retry with bounded backoff + jitter; UI exposes Retry save; delete runs `beginDelete`/`prepareDelete` (tombstone → cancel queued → await in-flight) before DB delete, then `disposeConversation` while store still has run keys (so in-flight streams abort), then drops store/query cache so an upsert cannot resurrect the row. Structured persistence logs: `saveStart`/`saveAck`/`saveFailed`, `deleteBegin`/`deleteCommit`/`deleteRollback`. Graceful shutdown: `interrupted`/`app_shutdown`, abort, bounded flush (`SHUTDOWN_FLUSH_TIMEOUT_MS` = 2s, `SHUTDOWN_SAVE_MAX_ATTEMPTS` = 3). Browser unload listeners are best-effort only; Electron main awaits a bounded renderer ack (`WINDOW_CLOSE_SHUTDOWN_TIMEOUT_MS` = 2.5s) before allowing/forcing window close. |
