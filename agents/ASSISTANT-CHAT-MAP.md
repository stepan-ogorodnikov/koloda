...
| Change retry behavior | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Retry) | `assistant/runs/use-run-orchestration.ts` (handleRetry), `runs/prepare-run-request.ts`, `runs/build-stream-request.ts` | Retry reuses the run ID; mode is preserved from the original run. AI profile/model/params come from the **current** selection, not the original request. |
