...
| `runs/` | Session (`RunController`), orchestration, engine host, command prep (`prepare-run-request` / `build-stream-request`), data access serialization (`data-access`), thin `engine.dispatch` adapter |
...
| *(root)* | Profile cascade hooks (`use-assistant-profile-selection`, `use-global-ai-profile-state`), data access resolver (`use-assistant-data-access`), runtime config, client, prompt templates |
...
| Change assistant data access (deck context and manifests) | docs/specs/ASSISTANT-DATA-ACCESS.md | `assistant/runs/data-access.ts` (serializers, manifest types, budget constants); `assistant/use-assistant-data-access.ts` (resolver hook); `runs/use-run-orchestration.ts`, `runs/prepare-run-request.ts`, `runs/build-stream-request.ts` (submit/retry wiring); `assistant/state/conversation-reducer.ts` (run `dataAccess`); `assistant/persistence/conversation-persistence-schema.ts`; `libs/ai/src/lib/prompts.ts` (`appendDataContext`); `@koloda/assistant` (`assistant-protocol.ts`, `assistant-engine.ts`, `conversation-runtime.ts` — opaque `RunDataAccessSnapshot`) | Always on — no consent, modes, or settings. Resolution happens exactly once per submit into a `{ context, manifest }` snapshot stored on the run. The request's `dataContext` is appended after the compiled system prompt — never a `{{placeholder}}`; prompt templates stay untouched. The manifest must match what the context text actually contains. Chat runs carry deck summaries only; card runs add write-target cards within budget (200-card cap, 8,000-char list budget). Retry replays the stored snapshot unchanged; only runs without one (restored pre-feature) resolve fresh, and the result is stored. `prepareRunRequest` / `buildStreamRequest` stay framework-free — reads happen in the hook via `queriesAtom` / query client at call time. Persistence validates the manifest; malformed ones fail as corrupt (optional field, no `schemaVersion` bump). |
