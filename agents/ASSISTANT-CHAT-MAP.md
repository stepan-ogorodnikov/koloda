...
| `runs/` | Session (`RunController`), orchestration, engine host, command prep (`prepare-run-request` / `build-stream-request`), card-run data access serialization (`data-access`), thin `engine.dispatch` adapter |
...
| *(root)* | Profile cascade hooks (`use-assistant-profile-selection`, `use-global-ai-profile-state`), card-run data access resolver (`use-assistant-data-access`), runtime config, client, prompt templates |
...
| Change conversation history rules | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Conversation History) | `assistant/state/conversation-reducer.ts` (getVisibleMessages), `state/assistant-messages.ts`, `runs/use-run-orchestration.ts`, `runs/prepare-run-request.ts`, `runs/build-stream-request.ts` | "What the user sees is what the model gets" (incl. partial fails, excl. card outputs and tool traffic). Tool rows live on the run record; follow-up requests do not resend them. |
| Change assistant data access (chat tools, card injection, manifests) | docs/specs/ASSISTANT-DATA-ACCESS.md | Chat tools: libs/ai `assistant-tools.ts` (specs + binder + output shaping) + `chat-stream.ts` (`fullStream` / `onToolEvent`); host executors (demo `ai-runtime`, Electron `ai-ipc` over `KolodaDb`); renderer `waitForStream` mapping; `@koloda/assistant` (`RunChunk` tool kinds, `assistant-execution-port.ts`, `conversation-runtime.ts`); `runs/build-stream-request.ts` (chat `tools` names); `runs/use-run-orchestration.ts` (no `resolveDataAccess` for chat); `assistant/state/conversation-reducer.ts` (`toolCalls`, `addToolCall` / `setToolCallResult`); `runs/assistant-event-to-action.ts`; libs/ai-react `ai-tool-activity.tsx`; `ui/use-assistant-message-renderer.tsx`; persistence `runSchema.toolCalls`. Card injection: `assistant/runs/data-access.ts`; `assistant/use-assistant-data-access.ts`; orchestration/prep (cards path only); `libs/ai/src/lib/prompts.ts` (`appendDataContext`); run `dataAccess` + opaque `RunDataAccessSnapshot`. | Always on — no consent, modes, or settings. Chat: names-only tools (`list_decks`, `get_deck_cards`); reach/egress at tool execution; budgets in tool output (200-card cap, 8,000-char list); activity visible on the run, not in follow-up history; retry re-executes fresh; old chat `dataAccess` snapshots are inert; no injected fallback (incapable models surface the provider error). Cards: submit-time `{ context, manifest }` snapshot, `dataContext` appended after the compiled system prompt — never a `{{placeholder}}`; retry replays the snapshot (pre-feature cards runs resolve fresh). Hosts bind executors; libs/ai has schemas/shaping only. `prepareRunRequest` / `buildStreamRequest` stay framework-free. Persistence: optional `toolCalls` / `dataAccess`, no `schemaVersion` bump; malformed values fail as corrupt. Crash-restore flips in-flight `running` tool calls to `error`. |
...
| Change retry behavior | docs/specs/ASSISTANT-CHAT-CONVERSATIONS.md (§Retry) | `assistant/runs/use-run-orchestration.ts` (handleRetry), `runs/prepare-run-request.ts`, `runs/build-stream-request.ts` | Retry reuses the run ID; mode is preserved from the original run. AI profile/model/params come from the **current** selection, not the original request. Retry is available for failed, canceled, and interrupted runs only — completed (success) runs are not retryable unless a separate regenerate feature is introduced. |
