...
Not the TypeScript apps and not `@koloda/db-pglite`.
...
- Repos: `repo/` — SQLite repos parallel to `@koloda/db-pglite` (plus AI secrets redaction/reconstruction). Owns `rusqlite` adapters (e.g. `FromSql` for `SettingsName`).
...
- PGlite / Drizzle PG schema — `@koloda/db-pglite`
