    ...
    <div className="my-2">
      <div className="sticky top-0 flex flex-row p-2 border-b-2 border-main bg-level-1">
        {(["untouched", "learn", "review", "total"] as const).map((type) => (
          <div className="w-1/4 text-center fg-table-head font-semibold" key={type}>
      ...
      {data.map((deck, i) => (
        <div className="flex flex-col pt-3 border-b-2 border-main" key={i}>
          {deck.title ? (
            ...
            <div className="px-2 text-center fg-level-4 font-semibold">{_(msg`lessons.table.columns.title.all`)}</div>
          )}
          <div className="flex">
            {(["untouched", "learn", "review", "total"] as const).map((type) => (
