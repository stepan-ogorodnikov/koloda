...
export type DispatchForConversation = (id: string, action: ConversationReducerAction) => void;
...
export function useConversationRuns(
  ...
  dispatch: Dispatch<ConversationReducerAction>,
  ...
  const handleStreamResult = useCallback((conversationId: string, result: StreamResult, runId: string) => {
    switch (result) {
      case "success":
        dispatchFor(conversationId, ["completeRun", { runId }]);
      ...
      case "error":
        break;
      case "aborted":
        dispatchFor(conversationId, ["cancelRun", { runId }]);
    ...
    async (conversationId: string, runId: string, request: ChatStreamRequest) => {
      try {
        ...
        const { streamResult, usage } = await streamChat(request, (chunk) => {
          currentText += chunk;
          dispatchFor(conversationId, ["updateAssistantText", { runId, text: currentText }]);
        });
        ...
        if (usage) dispatchFor(conversationId, ["setUsage", { runId, usage }]);
    ...
    async (conversationId: string, runId: string, request: CardGenerationStreamRequest) => {
      try {
        const result = await generate(request, (card) => {
          dispatchFor(conversationId, ["addCard", { runId, card }]);
        });
  ...
  const retryRun = useCallback(
    async (
      ...
      dispatch(["restartRun", { runId, request, templateFields, mode: effectiveMode, modelName }]);
