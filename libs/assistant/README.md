...
- Observability: `assistant-observability.ts` — structured `[assistant.transition]` logs keyed by conversation/run ids, command/event, prior/next status, termination reason, and save generation/attempt. Low-volume persistence events: `saveStart`/`saveAck`/`saveFailed`, `deleteBegin`/`deleteCommit`/`deleteRollback`; engine commands and `shutdown` are logged in `assistant-engine.ts`.
