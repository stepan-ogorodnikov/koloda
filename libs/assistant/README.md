...
- Engine: `assistant-engine.ts` — `createAssistantEngine` / `AssistantEngine`. Lazy per-conversation runtimes; typed `dispatch(command)` plus methods for arm / execute chat / execute generate / retry / cancel / `shutdownGracefully` / dispose.
