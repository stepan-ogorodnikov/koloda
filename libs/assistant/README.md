...
- Observability: `assistant-observability.ts` — structured lifecycle records keyed by conversation/run ids, optional host `requestId`, command/event, prior/next status, termination reason, and save generation/attempt.
  The default logger is a no-op; tests (and optional hosts) opt in with `setAssistantStructuredLogger`.
  Persistence events: `saveStart`/`saveAck`/`saveFailed`, `deleteBegin`/`deleteCommit`/`deleteRollback`; engine commands and `shutdown` are recorded in `assistant-engine.ts`.
...
- `docs/specs/ASSISTANT-CONVERSATION-LIST.md` — delete behavior the save queue must uphold
