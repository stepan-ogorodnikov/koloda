...
- Persistence: `conversation-persistence-host.ts`, `create-conversation-save-queue.ts`, `create-save-scheduler.ts` — engine-owned per-conversation serialized save queues; `SHUTDOWN_FLUSH_TIMEOUT_MS` (2000 ms) bounds the best-effort final flush on graceful shutdown.
