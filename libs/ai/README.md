...
- Reasoning levels: `providers/openrouter.ts` (API-provided); Ollama/Cloud from `/api/tags` `capabilities` including `thinking` (low/medium/high, default medium); LM Studio overlays `/api/v1/models` `capabilities.reasoning` onto `/v1/models` (picker hidden when the native list is unavailable or a model has no `allowed_options`); OpenCode Zen/Go join models.dev `effort` options by model id (`opencode` / `opencode-go`), with `resolveReasoningLevelsForModel` as prefix fallback when the catalog is unavailable or the id is missing.
