...
- Assistant tools: `assistant-tools.ts` — specs for the names-only tools (`list_decks`, `list_templates`, `list_algorithms`, `get_deck`, `get_template`, `get_deck_cards`, `propose_cards`), input binding, output shaping and budgets; `assistant-tool-executor.ts` — the shared find-deck → find-template → shape executor, awaiting a host-injected `AssistantToolDataSource` (web SQLite, Electron NAPI). No I/O in this module — hosts bind data sources.
