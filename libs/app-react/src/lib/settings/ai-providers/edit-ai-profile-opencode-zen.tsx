import { aiProfileValidation, opencodeZenSecretsValidation } from "@koloda/ai";
...
const formSchema = opencodeZenSecretsValidation.extend({
  title: aiProfileValidation.shape.title,
});

type FormValues = z.infer<typeof formSchema>;

export function EditAIProfileOpencodeZen({ profile, onSubmit, isPending, error }: EditAIProfileFormProps) {
  const { _ } = useLingui();
  const apiKey = profile.secrets?.provider === "opencodeZen" ? profile.secrets.apiKey : "";
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      onSubmit({
        title: value.title,
        secrets: { provider: "opencodeZen", apiKey: value.apiKey },
      });
    },
  });
