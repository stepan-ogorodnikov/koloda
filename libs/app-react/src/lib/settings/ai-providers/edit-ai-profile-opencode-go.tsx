import { aiProfileValidation, opencodeGoSecretsValidation } from "@koloda/ai";
...
const formSchema = opencodeGoSecretsValidation.extend({
  title: aiProfileValidation.shape.title,
});

type FormValues = z.infer<typeof formSchema>;

export function EditAIProfileOpencodeGo({ profile, onSubmit, isPending, error }: EditAIProfileFormProps) {
  const { _ } = useLingui();
  const apiKey = profile.secrets?.provider === "opencodeGo" ? profile.secrets.apiKey : "";
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      onSubmit({
        title: value.title,
        secrets: { provider: "opencodeGo", apiKey: value.apiKey },
      });
    },
  });
