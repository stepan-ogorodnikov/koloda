...
export function CardsTableFilters({ filters, setFilters }: CardsTableFiltersProps) {
  const { _ } = useLingui();

  const resetFilters = () => setFilters({ state: [], dueAt: { isOverdue: false, isNotDue: false } });
        ...
        <Dialog.Content variants={{ class: "gap-4 w-60" }}>
          <div>
            ...
            {VALUES.map(({ t }, i) => (
              ...
              >
                <Checkbox.Indicator />
                <Checkbox.Label>{_(t)}</Checkbox.Label>
              </Checkbox>
            ))}
          </div>
          <div>
