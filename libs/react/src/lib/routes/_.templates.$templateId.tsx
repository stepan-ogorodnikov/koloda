import { queriesAtom, Template, templatesQueryKeys } from "@koloda/react";
...
export const Route = createFileRoute("/_/templates/$templateId")({
  ...
  loader: ({ context: { queryClient, queries }, params: { templateId } }) => {
    ...
    queryClient.ensureQueryData({ queryKey: templatesQueryKeys.detail(templateId), ...getTemplateQuery(templateId) });
  },
});
...
function TemplateRoute() {
  ...
  const { data } = useQuery({ queryKey: templatesQueryKeys.detail(templateId), ...getTemplateQuery(templateId) });
